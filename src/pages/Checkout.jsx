  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setErrorMsg(data.message || 'Error processing checkout');
      }
    } catch (error) {
        ...
        <div className="flex flex-col lg:flex-row gap-12">
          ...
          <div className="w-full lg:w-2/3">
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              {/* Payment Details */}
              <div className="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
                <h2 className="text-xl font-medium text-primary mb-6 uppercase tracking-wider text-sm border-b pb-4">Payment Details</h2>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="col-span-1 md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Cardholder Name</label>
                    <input name="cardName" required type="text" className="w-full p-3 border border-gray-300 rounded focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" />
                  </div>
                  <div className="col-span-1 md:col-span-2">
                    <label className="block text-sm font-medium text-gray-700 mb-2">Card Number</label>
                    <input name="cardNumber" required type="text" className="w-full p-3 border border-gray-300 rounded focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" placeholder="0000 0000 0000 0000" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Expiry Date</label>
                    <input name="expiry" required type="text" className="w-full p-3 border border-gray-300 rounded focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" placeholder="MM/YY" />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">CVV</label>
                    <input name="cvv" required type="text" className="w-full p-3 border border-gray-300 rounded focus:border-accent focus:ring-1 focus:ring-accent outline-none transition-colors" placeholder="123" />
                  </div>
                </div>
              </div>
