      ...
      <nav className={`fixed w-full z-40 transition-all duration-300 ${isScrolled ? 'bg-white/90 backdrop-blur-md shadow-sm py-4' : 'bg-transparent py-6'}`}>
        ...
        <div className={`md:hidden absolute top-full left-0 w-full bg-white shadow-lg transition-all duration-300 ease-in-out ${isMobileMenuOpen ? 'max-h-[400px] opacity-100 py-4' : 'max-h-0 opacity-0 overflow-hidden py-0'}`}>
          <div className="flex flex-col space-y-4 px-4">
            ...
            <div className="pt-2 pb-6 space-y-4 border-t border-gray-100 mt-2">
              ...
              {isLoggedIn ? (
                <button 
                  ...
                  className="flex items-center space-x-2 text-red-500 hover:text-red-600 w-full py-2 bg-red-50 rounded-lg px-4 transition-colors"
                >
                  <LogOut size={20} />
                  <span className="font-medium uppercase tracking-wide text-sm">Logout from Account</span>
                </button>
              ) : (
                <button onClick={() => { setIsMobileMenuOpen(false); navigate('/login'); }} className="flex items-center space-x-2 text-primary hover:text-accent w-full py-2">
                  <User size={20} />
                  <span className="font-medium uppercase tracking-wide">Login / Register</span>
                </button>
              )}
            </div>
          </div>
              ...
              cartItems.map((item) => (
                <div key={item.cartId} className="flex gap-4">
                  <div
                    ...
                    onClick={() => { setIsCartOpen(false); navigate(`/product/${item._id || item.id}`); }}
                  >
